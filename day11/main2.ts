...
class Monkey {
  ...
  updateWorryLevel(item: number, lcm: number) {
  ...
  inspectItems(monkeys: Monkey[], lcm: number) {
    while (this.items.length > 0) {
      let item = this.items.shift()!;
      item = this.updateWorryLevel(item, lcm);
      const nextMonkey =
...
// LCM here is just product since all
// numbers are mutually prime :P
const LCM = monkeys.reduce((prod, monkey) => prod * monkey.testNumber, 1);

const NUMBER_OF_ROUNDS = 10000;

for (let i = 0; i < NUMBER_OF_ROUNDS; i++) {
  monkeys.forEach((m) => m.inspectItems(monkeys, LCM));
}
